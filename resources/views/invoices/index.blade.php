									...
									<table class="table table-condensed">
										...
										<tbody>
											@forelse (Auth::user()->payments as $payment)
												<tr>
													<td>{{$payment->invoice->series->prefix ?? ''}}-{{ str_pad($payment->invoice->id, 5,'0', STR_PAD_LEFT) }}</td>
													<td>{{ $payment->invoice->due_date }}</td>
													<td>@if ($payment->invoice->status =='Draft')
															<button class="btn btn-warning btn-flat"> Pending</button>
														@endif
														@if ($payment->invoice->status =='Created')
															<button class="btn btn-dark btn-flat"> Pay</button>
														@endif
                                                    </td>
                                                    <td>
                                                        <a href="{{ route('payments.create',$invoice->id) }}" class="btn btn-info"> Pay invoice </a>
